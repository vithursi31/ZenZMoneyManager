...
- **One active currency per user** (MVP). A money-bearing row either **carries** `currency` or **derives** it from its account, and which one follows what the row is: a record of money that moved carries its own copy (`Transaction`, `Account`, `SavingsGoal`, `Loan`) because that is history and cannot be re-derived; a *rule about* an account reads it off that account at request time (`Budget`, `RecurringTransaction`) because a stored copy could only go stale. Those per-row copies are the seam that makes multi-currency additive later. No implicit FX conversion.
...
- **No stored balance anywhere; a user may hold multiple accounts.** An account is auto-provisioned (unnamed) at first use, and a user may add more via the account API — each carries an optional `name` and a lifecycle `status` (`ACTIVE`/`INACTIVE`/`DELETED`; delete is soft, and a user must always keep at least one `ACTIVE` account). None has `opening_balance`/`current_balance` columns. **Ledger writes still resolve one implicit account server-side**: `Transaction.accountId` and `RecurringTransaction.accountId` are resolved to the caller's oldest `ACTIVE` account, never accepted from a request body — there is still no way for a client to choose which of their accounts a transaction lands in. The user-facing figure is the **monthly position**: `Σ INCOME − Σ EXPENSE` over `[month start, next month start)` in the user's timezone, computed on read across all the user's transactions regardless of account, carried forward nowhere — see [domain-documentation.md §1.10](docs/domain/domain-documentation.md#110-monthly-position-invariant). **Only real rows are summed**: an upcoming recurring payment is projected from its template on read (`GET /api/v1/recurring/upcoming`), never pre-written as a pending row, and there is deliberately no `PENDING`/`PAID` status on a transaction — a status that flips only because a date passed is derivable from the date, and storing it would put an "is it real yet" filter on every aggregate. **`transaction.status` exists but is lifecycle, not settlement** (`ACTIVE`/`DELETED`, added 2026-09-01): deleting is soft, because a chat turn and a goal contribution both record the transaction they refer to. Deletion is derivable from nothing, which is the only reason that column earns the very filter cost this rule refuses to pay for settlement — **never add `PAID` to it**. The consequence is a standing obligation: **every aggregate over `transaction` must filter `status = ACTIVE`**, and a new one that forgets keeps counting money the user removed, silently, in the monthly position. Consequently **there is still no `TransactionType.TRANSFER`** (that needs an explicit account choice on the write path, which doesn't exist yet) and no reconciliation or net worth. Per-transaction account selection and transfers are the remaining, unscheduled slice of F-F.1.
- **Enums are `@Enumerated(EnumType.STRING)` in `VARCHAR(50)`**, and the enum types live in `common/domain` — not in `core`.
- **Every user-facing message is a key, resolved in the caller's language at the boundary.** A service throws `new BadRequestException(Msg.CATEGORY_DUPLICATE, name)`, never a sentence; `GlobalExceptionHandler` renders it. `errorCode` is unaffected and stays the client's contract — see the *Messages and languages* section below and [domain-documentation.md §0.5](docs/domain/domain-documentation.md#05-user-facing-messages-are-localised).
...
- **[ApiResponse&lt;T&gt;](svcs/common/src/main/java/com/zenzmoney/common/dto/ApiResponse.java)** — the envelope for **every** JSON response: `{status, data, message, errorCode}`. Build with `ApiResponse.success(data)` / `ApiResponse.error(statusCode)`. Controllers return `ResponseEntity<ApiResponse<T>>`. Never return a bare entity or map. `error` takes a `StatusCode`, never a string — there is deliberately no way to mint a code from a literal. The two-arg `error(statusCode, message)` overload carries an **already-rendered** message and is for boundary code only (the three writers listed under *Messages and languages*); everywhere else there is no locale to render with.
...
- **[StatusCode](svcs/common/src/main/java/com/zenzmoney/common/status/StatusCode.java) is the single fact about a failure** — its wire `errorCode`, its HTTP status, and its default message, in one immutable value. Codes live in exactly two registries: [StatusCodes](svcs/common/src/main/java/com/zenzmoney/common/status/StatusCodes.java) (the five the boundary machinery itself needs) and [ServiceCodes](svcs/common/src/main/java/com/zenzmoney/common/status/ServiceCodes.java) (everything else, banded by concern). `ServiceCodes` extends `StatusCodes`, so core code imports **one** registry. Override the message at a call site with `SC_X.with("…")` — same code, same status, different text.
  ...
  **A `StatusCode` carries two kinds of message and they are not interchangeable.** `SC_X.with(Msg.KEY, args)` is what the *user* reads — a bundle key, rendered in their language. `SC_X.with("some text")` is a **diagnostic**: English, log-only, and it never reaches the client, so it is the right place for a provider or JWT-library detail. Reaching for the string form for user-facing copy silently downgrades the response to the code's generic sentence.
...
The app answers in the user's language (F-1.26). One rule decides where everything goes:
**a service throws a key; the boundary renders it.**

```
throw new BadRequestException(Msg.CATEGORY_DUPLICATE, name);   // service: key + args, no Locale
```

- **[Msg](svcs/common/src/main/java/com/zenzmoney/common/i18n/Msg.java)** is the key registry, grouped by the same bands as `ServiceCodes`. **A key is not an error code**: keys are one-per-rejection, codes stay coarse. ~76 rejections share `E1013`/`E1010` and are told apart by their key, which is why translating them did not force a redraw of the band map. Promoting one to its own `E11xx` stays an independent, client-visible decision.
- **Text lives in [`core/src/main/resources/i18n/`](svcs/core/src/main/resources/i18n/)** — `messages.properties` is the base (English); `messages_<tag>.properties` translates it. **12 bundles ship today**: `en`, `zh-CN`, `zh-TW`, `fr`, `es`, `pt`, `de`, `it`, `ru`, `ja`, `ko`, `si` — Chinese twice, because the script changes the text. `common` holds keys and no text; it has no Spring dependency. **Everything but English is machine-assisted and unreviewed** — see the header of each bundle.
- **Adding a message:** a constant in `Msg`, a line in `messages.properties`, and a line in every other bundle. `MessageBundleCompletenessTest` fails if you skip one — it also checks that every registry `StatusCode` has an `error.code.<CODE>` generic and that every `{0}` pattern still interpolates.
- **Adding a language:** a tag in `zenzmoney.i18n.available-languages` (in [application.properties](svcs/core/src/main/resources/application.properties), env var `AVAILABLE_LANGUAGES`) and a matching `messages_<tag>.properties` — dashes become underscores, so `zh-TW` needs `messages_zh_TW.properties`. Nothing else, no Java change. That one property is also the allowlist for `Accept-Language` and for the `language` field on `PUT /me` / onboarding, so a tag outside it is refused on write rather than stored.
- **The config names bundles exactly; matching a caller's tag is what is lenient.** A caller is matched on **language + script**: region is ignored (`fr-CA` → `fr`, `pt-BR` → `pt`), script is not, because for Chinese it changes the text — `zh-TW`/`zh-HK`/`zh-MO`/`zh-Hant` → Traditional, `zh-CN`/`zh-SG`/`zh-Hans`/bare `zh` → Simplified. The region→script map in [SupportedLanguages](svcs/core/src/main/java/com/zenzmoney/core/util/SupportedLanguages.java) is gated on `zh` so `en-SG` does not come out Chinese. **One matcher, used everywhere** — [SupportedLocaleResolver](svcs/core/src/main/java/com/zenzmoney/core/i18n/SupportedLocaleResolver.java) replaces `AcceptHeaderLocaleResolver`'s own list-matching rather than wrapping it, because the stock one collapses `zh-Hant` to the default before anything downstream can see the script.
...
- **Only five places render a message**, and they are all boundary code: [GlobalExceptionHandler](svcs/core/src/main/java/com/zenzmoney/core/web/advice/GlobalExceptionHandler.java), [AccessDeniedAdvice](svcs/core/src/main/java/com/zenzmoney/core/web/advice/AccessDeniedAdvice.java) (where `@RolesAllowed` actually refuses a caller), [JwtAuthenticationFilter](svcs/core/src/main/java/com/zenzmoney/core/web/filter/JwtAuthenticationFilter.java) (401s written before the dispatcher runs), the access-denied handler in [SecurityConfig](svcs/core/src/main/java/com/zenzmoney/core/config/SecurityConfig.java) (the filter-chain fallback for the same thing), and [ChatText](svcs/core/src/main/java/com/zenzmoney/core/i18n/ChatText.java) called from [ChatController](svcs/core/src/main/java/com/zenzmoney/core/web/controller/ChatController.java) — the only one on a *successful* response, because chat is the only place the app writes a sentence the user reads when nothing went wrong. A sixth would be a bug: it means something below the web layer acquired a `Locale`.
...
> **Chat replies are localised** (2026-08-29). `ChatService` stores a `Msg` key in `chat_message.content` and returns it on the wire; [ChatText](svcs/core/src/main/java/com/zenzmoney/core/i18n/ChatText.java) renders it in `ChatController` — a fifth boundary renderer, and the only one on a *successful* response. Content that is not a key (the user's own message, and prose the model wrote for F-1.16) passes through, which is what the `chat.` prefix distinguishes. Because the key is what is stored, replaying a conversation answers in whatever language the reader uses *now*.
>
> **Still English, deliberately:** the LLM prompt language (the pending question is rendered with `Locale.ENGLISH` before it reaches the model, the same rule the logs follow) and the Thymeleaf templates. Tracked on **F-1.26** in [docs/features-list.md](docs/features-list.md).
>
...
- `V8__payment_method.sql` — nullable `payment_method` on `transaction` and `recurring_transaction`, each with a CHECK matching the enum
- `V9__recurring_currency_from_account.sql` — drops `recurring_transaction.currency`; a template reads it off its account
- `V11__chat_recurring_link.sql` — `chat_message.recurring_id`, so a chat turn records the template it created
- `V12__transaction_soft_delete.sql` — `transaction.status` (`ACTIVE`/`DELETED`) + CHECK; `idx_transaction_user_date` becomes partial on `ACTIVE`
...
**Chat model provider:** `zenzmoney.llm.provider` is `ollama` (default, self-hosted) or `gemini` (hosted API). Both implement [LlmExtractionClient](svcs/core/src/main/java/com/zenzmoney/core/service/llm/LlmExtractionClient.java) and [LlmAdviceClient](svcs/core/src/main/java/com/zenzmoney/core/service/llm/LlmAdviceClient.java); the beans are gated by `@ConditionalOnProperty` so exactly one pair exists at runtime and `ChatService` never learns which. **`gemini` requires `GEMINI_API_KEY` and fails the boot without it** — an app that starts and then fails every chat message looks healthy and is not. The default is `ollama` because it needs no secret, so a fresh clone and an offline machine both work. **Switching to `gemini` changes what leaves the machine** — the user's own chat text, not just category names — see [domain §3.7](docs/domain/domain-documentation.md).
...
[logback-spring.xml](svcs/core/src/main/resources/logback-spring.xml) writes to `${DEPLOYMENT_DIRECTORY:-.}/logs/ZenZMoneyManager/` in **every** profile except `test` (which is console-only) — `./logs/ZenZMoneyManager` locally, `/app/logs/ZenZMoneyManager` in the container. Console stays attached everywhere too, so `docker compose -f docker-compose.prod.yml logs -f app` is not empty.
...
Each file rolls on a **daily boundary or at 10MB**, whichever comes first, and is gzipped into `logs/ZenZMoneyManager/archived/`. Live files and compressed history sit in **separate directories** on purpose, so moving archives off-host is a plain `mv archived/*.gz <dest>` that cannot touch the file the app is appending to. **`archived/` does not exist until the first roll** — a fresh deployment has no archive directory for up to a day, which is normal.
...
- **A new column on a ledger row is nullable unless the past can honestly answer it.** `transaction.payment_method` is null for every row written before the field existed, because a backfilled default would be a guess shown back to the user as their own answer. Same rule for anything added to `transaction` or `recurring_transaction` later.
...
- **User-facing text comes from a message key, never from a string literal.** `new BadRequestException("Category not found")` answers the client with a generic "Not found" — the literal is a log-only diagnostic. Use `Msg.CATEGORY_NOT_FOUND`. Same rule for `StatusCode.with(...)`. See *Messages and Languages*.
...
- **Never call the Gemini API unless the user asks.** The project runs on a **free tier with a hard quota**, and a burned quota blocks the user's own work. That rules out the obvious "let me just check" moves: `ExtractionPromptEvalTest` with `-Dllm.eval=true`, ad-hoc scoring scripts over the fixture set, and sending chat messages through a running app to see what comes back. Verify prompt and client changes by **reading** — the code, `llm.log` from calls the user already made, and unit tests against a stubbed `WebClient` (`GeminiExtractionClientTest`), none of which touch the network. When a change genuinely cannot be verified without a live call, say so and let the user spend the quota deliberately. The same restraint applies to any metered provider added later; a self-hosted Ollama is the one that costs nothing but CPU.
...
| A new user-facing message, message key, or language | The key in `Msg` + a line in **every** bundle — `MessageBundleCompletenessTest` is the guard and needs no editing. Add a rendering assertion only when the message takes arguments | `svcs/core/src/test/java/com/zenzmoney/core/i18n/...` |
