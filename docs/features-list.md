...
| A `PENDING`/`PAID` status on a transaction | An upcoming payment is projected from its recurring template, not written ahead as a row (F-1.7). A status that flips only because a date passed is derivable from the date, and storing it would put an "is it real yet" filter on every total. Revisit if the user is ever given an action the clock cannot infer — *skip this month*, *this did not go through*, or a differing amount. |
...
| ID | Feature | Description | Status |
|---|---|---|---|
| **F-1.3** | Expense transactions | Record an expense with amount, category, date, payee/merchant, notes, and how it was paid (cash / card / bank transfer / wallet — a label on the entry, not an account; see [domain-documentation.md §1.6](domain/domain-documentation.md#16-transaction)). | 🚧 |
...
| **F-1.7** | Recurring income, expenses & subscriptions | One model for everything that repeats — salary, rent, utilities, and subscriptions (streaming, music, gym): amount/cost, repeat or billing frequency, next due or renewal date, how it is paid, and an optional **free-trial end date**. Generates transactions into the month they fall in, consistent with F-1.2, and drives the reminders in F-1.20. Includes the **upcoming-payments** list (default 3 days ahead, up to 90) that the entry and home screens read — **projected from the templates on every read, never pre-written as pending rows**, so nothing the user has not paid yet is counted by the monthly position or a budget. See [domain-documentation.md §1.8](domain/domain-documentation.md#18-recurringtransaction). | 🚧 |
...
| ID | Feature | Description | Status |
|---|---|---|---|
| **F-1.11** | Chat-based transaction entry | Record income and expenses in plain language — *"I spent 15 at Keells for tea"*, *"I received my salary of 3000"*. The model behind it is selected by `zenzmoney.llm.provider` — **`ollama`** (self-hosted, the default) or **`gemini`** (Google's hosted API); either extracts intent / type / amount / category-guess / date-phrase / payee / note, and the two are interchangeable because the contract is the prompt's, not the vendor's. **Under `gemini` the user's own message text leaves the machine** — see [domain §3.7](domain/domain-documentation.md).  the **backend** resolves each field — amount → minor units in the active currency, guess → a real [Category](domain/domain-documentation.md#15-category), date-phrase → an exact timestamp in the user's timezone, payee-name → a [Payee](domain/domain-documentation.md#15b-payee) row. **A message the model reads completely is recorded straight away** and can be reversed with `POST /api/v1/chat/undo` — the write is made *reversible* rather than *conditional* (amended 2026-08-29; see [domain §3.7](domain/domain-documentation.md)). Missing information produces a **clarifying question** — one field at a time, answered by typing, with no suggestion chips — and the answer completes the entry on the next turn. The model is given the **last two exchanges** as context, so *"one ticket 50 for snacks 10"* reads as the price of the outing just asked about; amounts in an answer are **summed into that one entry**, while a fresh message naming several amounts still splits. **One message may record several entries** (*"$28 on coffee, $350 on groceries and $120 on fuel"* is three), and a message describing something that **repeats** (*"Netflix 15 every month"*) creates a [recurring template](domain/domain-documentation.md#18-recurringtransaction) under F-1.7 rather than a single row. Conversations are logged (`ChatMessage`) so a saved transaction traces back to what was typed. See [domain-documentation.md Part 3](domain/domain-documentation.md#part-3--ingestion--ai-mvp). | 🚧 |
...
| **OQ-5** | F-1.11 | Sending chat to a hosted model (`provider=gemini`) puts the user's own message text — notes, payees, amounts — in a third party's hands. Does that need explicit user consent, or a per-user setting, before real users? Today it is a deployment-wide switch with no disclosure in-app. | Documented in [domain §3.7](domain/domain-documentation.md); **nothing enforces it**. Default stays `ollama` so a deployment must opt in. | Product / Legal |
| **OQ-6** | F-1.11 | A user answering "how much?" with several parts (*"one ticket 50 for snacks 10"*) only sums when the model returns one item per amount. When it returns a single merged item the second amount is silently dropped. Summing every number in the message instead would break on quantities (*"2 tickets at 50"* → 52). | Unresolved. The backend sums what the model splits; when it does not split, the entry is short. | Engineering |
| **OQ-7** | F-1.16 | The assistant's prose is returned to the user unchecked — it has been observed inventing figures that appear nowhere in the snapshot. The prompt forbids it; nothing enforces it. | Open. The fix is a backend check: every number in the reply must appear in the snapshot, or the reply is discarded. | Engineering |
| **OQ-8** | F-1.11 | Splitting a bill (*"lunch with team, split 4 ways"*) — one transaction at the user's share, or model the participants? | Chosen shape: one transaction at the user's share. Not implemented; chat currently records the full amount. | Product |
...
| 2026-08-29 | **Chat direct entry (F-1.11).** The draft→confirm gate is replaced by a **reversible write**: a complete, above-threshold reading is recorded immediately and undone with `POST /api/v1/chat/undo`; a sub-threshold one still needs a confirm. Extraction now returns **one item per amount**, so a single message can record several entries, and a repeat phrase creates a **recurring template** (F-1.7) instead of a one-off row. Suggestion chips removed — the question is answered by typing, which is also the only path voice entry (F-1.12) could take. **Chat replies are now localised** like every other message (F-1.26), stored as keys in `chat_message.content` and rendered at the boundary. |
| 2026-08-21 | **Recurring/subscription slice filled in.** Added the **upcoming-payments** projection to F-1.7 (`GET /api/v1/recurring/upcoming`, default 3 days, max 90) and recorded the decision *against* pre-writing pending ledger rows in [out of scope](#out-of-scope). Added **`paymentMethod`** to F-1.3/F-1.7 — the entry screen's "Payment" row, a label on the transaction rather than an account type. Recurring date math now advances in the owner's timezone rather than UTC (a `nextRunDate` at local midnight previously anchored to the wrong day and generated into the neighbouring month), creating a template already due posts its first occurrence in the request, and generation runs every 15 minutes instead of hourly. **Dropped `recurring_transaction.currency`** — a template is a rule about an account and now reads the currency off it, the same way a budget has since 2026-08-18; the API still returns `currency`, resolved on read, and generated transactions keep their own copy because a ledger row records what the money was. |
